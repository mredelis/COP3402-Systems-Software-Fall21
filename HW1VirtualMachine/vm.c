...
int main(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    printf("Invalid command line arguments. To execute the program ./a.out input.txt\n");
    exit(EXIT_FAILURE);
  }

  // Try to open input file and load the program into PAS
  if((ifp = fopen(argv[1], "r")) != NULL)
  {
    // Read and store instructions in PAS. 
    while((fscanf(ifp, "%d %d %d", &OP, &L, &M) != EOF)) {
      PAS[IC++] = OP;
      PAS[IC++] = L;
      PAS[IC++] = M;
    }

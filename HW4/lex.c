...
lexeme *lexanalyzer(char *input, int printTokensFlag)
{
  ...
  //driver.c reads in input file with program in PL/0 into a char array and appends '\0' 
  //when finish reading
  while(input[i] != '\0') 
  {
  ...
  //If compiler directive -l, print lexeme table and tokens list
  if(printTokensFlag == 1)
    printtokens();
